...
function scrambleText(target, active) {
  if (!active) {
    return target;
  }
  ...
  return target
    ...
    .map((character, index) => {
      ...
      return Math.random() > 0.42 && index < active ? character : pick(GLYPHS);
    })
    .join("");
}
...
function App() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = window.setInterval(() => {
      setScrambleFrame((value) => (value + 1) % 20);
    }, 70);

    return () => window.clearInterval(intervalRef.current);
  }, [phrase, isReducedMotion]);

  const visiblePhrase = isReducedMotion ? phrase : scrambleText(phrase, scrambleFrame * 2);
